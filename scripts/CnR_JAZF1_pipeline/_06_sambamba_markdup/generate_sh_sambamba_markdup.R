setwd("/Users/chris/Desktop/20240318_CnR_batch2")

library(tidyverse)

##### module load bowtie2/2.5.1 samtools/1.17

#
fastq_list_filename <- "20240318_CnR_batch2_fastq_list.txt"
df <- read_tsv(file.path("input", "batch2", fastq_list_filename))
fastq_folder <- "cnr_batch2"
output_pipeline_dir <- "cnr-pipeline_batch2-GRCh38_PE"
script_pipeline_dir <- "CnR_batch2_pipeline"
workdir <- "/home/chris11/projects/def-stbil30/chris11/20240318_CnR_batch2"

#
header_sh <- c("#!/bin/sh",
               "#SBATCH --time=3:00:00",
               "#SBATCH --nodes=1",
               "#SBATCH --ntasks-per-node=1",
               "#SBATCH --cpus-per-task=4",
               "#SBATCH --mem-per-cpu=8G",
               "#SBATCH --account=def-stbil30",
               "#SBATCH --mail-user=christophe.tav@gmail.com",
               "#SBATCH --mail-type=ALL")

#
# bowtie2_path <- "/cvmfs/soft.computecanada.ca/easybuild/software/2020/avx2/Core/bowtie2/2.5.1/bin/bowtie2"
# samtools_path <- "/cvmfs/soft.computecanada.ca/easybuild/software/2020/avx2/Core/samtools/1.17/bin/samtools"
# bowtie2_hg38_idx <- "/home/chris11/projects/def-stbil30/chris11/20230918_CnR_pipeline_demo/input/bowtie2_genome_index_homo_sapiens/Homo_sapiens.GRCh38"
sambamba_path <- "/cvmfs/soft.computecanada.ca/easybuild/software/2020/Core/sambamba/0.8.0/sambamba"

#
nThreads <- 8

for (i in 1:nrow(df)) {
  sample_name <- df$sample_name[i]
  # message("# ", i, " | ", sample_name)
  
  #
  output_bowtie2_dir <- file.path(workdir, "output", output_pipeline_dir, "alignment", sample_name)
  
  #
  bam_sorted_filename <- paste0(sample_name, ".sorted.bam")
  bam_sorted_filepath <- file.path(output_bowtie2_dir , bam_sorted_filename)
  
  #
  bam_markdup_filename <- paste0(sample_name, ".sorted.markdup.bam")
  bam_markdup_filepath <- file.path(output_bowtie2_dir , bam_markdup_filename)
  
  # sambamba markdup
  call_sambamba_markdup <- paste(sambamba_path, "markdup",
                                 "-t", nThreads,
                                 "--sort-buffer-size=8192",
                                 "--io-buffer-size=1024",
                                 bam_sorted_filepath,
                                 bam_markdup_filepath)

  # generate scripts
  file_sh <- file.path("scripts", script_pipeline_dir, "06_sambamba_markdup", "batch_sh",
                        paste0("sambamba_markdup_", sample_name, ".sh"))
  message("sbatch ", file_sh)
  fileConn <- file(file_sh)
  writeLines(c(header_sh, "\n", call_sambamba_markdup), fileConn)
  close(fileConn)
  # message(" > Saved in ", file_sh)
}
