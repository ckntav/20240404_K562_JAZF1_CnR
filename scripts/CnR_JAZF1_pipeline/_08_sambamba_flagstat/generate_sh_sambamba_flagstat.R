...
workdir <- "/home/chris11/projects/def-stbil30/chris11/20240318_CnR_batch2"
...
for (i in 1:nrow(df)) {
  ...
  bam_flagstat_filepath <- gsub(".bam", ".flagstat", bam_filepath)
  ...
  bam_markdup_flagstat_filepath <- gsub(".bam", ".flagstat", bam_markdup_filepath)
  ...
  bam_filtered_flagstat_filepath <- gsub(".bam", ".flagstat", bam_filtered_filepath)
  
  # sambamba flagstat bam
  call_sambamba_flagstat_bam <- paste(sambamba_path, "flagstat",
                                      bam_filepath,
                                      ">", bam_flagstat_filepath)
  
  # sambamba flagstat markdup
  call_sambamba_flagstat_markdup <- paste(sambamba_path, "flagstat",
                                          bam_markdup_filepath,
                                          ">", bam_markdup_flagstat_filepath)
  
  # sambamba flagstat filtered
  call_sambamba_flagstat_filtered <- paste(sambamba_path, "flagstat",
                                           bam_filtered_filepath,
                                           ">", bam_filtered_flagstat_filepath)

  # generate scripts
  file_sh <- file.path("scripts", script_pipeline_dir, "08_sambamba_flagstat", "batch_sh",
                        paste0("sambamba_flagstat_", sample_name, ".sh"))
  message("sbatch ", file_sh)
  fileConn <- file(file_sh)
  writeLines(c(header_sh, "\n",
               call_sambamba_flagstat_bam, "\n",
               call_sambamba_flagstat_markdup, "\n",
               call_sambamba_flagstat_filtered), fileConn)
  close(fileConn)
